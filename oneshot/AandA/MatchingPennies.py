    ...
    **Please change the name of this class to match your agent name**
    ...
    def best_subset(self, needs, offers, plist, quantity_cost_tradeoff=0.95):
        ...
        for i, partner_ids in enumerate(plist):
            ...
            total_contracts_cost = sum(offers[p][UNIT_PRICE]*offers[p][QUANTITY] for p in partner_ids)
            penalty = 0
            ...
            total_profit = (self.p * self.q - total_contracts_cost - penalty) if self.awi.level == 1 else (total_contracts_cost - self.p * self.q - penalty)

            # Normalize total_profit and diff
            total_profit_normalized = (total_profit - min_utility) / (max_utility - min_utility) if (max_utility - min_utility) != 0 else 0
            diff_normalized = (diff - min_diff) / (max_diff - min_diff) if (max_diff - min_diff) != 0 else 0

            # Calculating loss with normalized values
            loss =  quantity_cost_tradeoff * diff_normalized - (1 - quantity_cost_tradeoff) * total_profit_normalized
    ...
    def counter_all(self, offers, states):
        ...
        for needs, all_partners, issues in [
            ...
            best_diff, best_indx = self.best_subset(
                ...
                needs, offers, plist, quantity_cost_tradeoff= (1.0 - self.q/100)
            )
    ...
    def step(self):
        ...
        uprice = self.todays_spent/self.todays_contracts_num if not(self.todays_contracts_num == 0) else 0
        if(self.verbose):
            print(f"Today I need {self.q} contracts, I managed to get {self.todays_contracts_num} contracts with the price of {uprice} each. Today I accumulated a profit of {self.ufun.from_contracts(self.contracts)}.")
            print(f"Today's best price is {self.best_price}")
            print(f"Today's exorgenous contracts are at {self.p/self.q} for each of the {self.q} items")
    ...
    def on_negotiation_success(self, contract: Contract, mechanism: OneShotAWI) -> None:  # type: ignore
        ...
        self.todays_spent += contract.agreement ['unit_price'] * contract.agreement['quantity']
